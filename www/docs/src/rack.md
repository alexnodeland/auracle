...
audio; amber carries modulation, and its cable says what it lands on.</figcaption>
</figure>
...
- **Modules** are plates with a title, a ⋯ menu, and their controls. Knobs wear a
  value arc and read in **musical units** — `840 Hz`, `24 ms`, `−6.0 dB`,
  `+12 ¢`, `8.23 Hz` — not normalized 0–1, because you are being asked to make a
  musical judgement and `0.63` is not one.
- **Jacks** are small rings labelled `in` / `out`. Their colour tells you the
  signal kind, and the type system decides what can legally connect.
...
- **Modulation cables** are amber, and each one **ends in a named destination** —
  ...
  modulator's actual rate, so you can see a 0.2 Hz sweep before you hear it.
...
which is the right behaviour but does mean a very large patch shows as bare
plates until you zoom in.
...
Drag a knob, or focus it and use <kbd>↑</kbd>/<kbd>↓</kbd> — hold <kbd>Shift</kbd>
for fine. Click a selector (`saw`, `square`, `−2 oct`) to cycle it.

Every edit is a **one-site write at that knob's trace address**. The patch is
re-rendered and re-vetted off the main thread before it can be auditioned, and the
live instrument is re-patched immediately so held notes keep sounding.

Edits are staged. The toolbar's **commit** inserts the result as a new candidate,
leaving the original intact. <kbd>⌘Z</kbd> / <kbd>⇧⌘Z</kbd> undo and redo.

**my edit is better** is a separate claim. Ticking it also teaches the model an
"edit beat original" duel — a real observation, and one the
[TRUST tab scores separately](./views/taste.md#trust--is-its-confidence-honest)
from duels you actually listened to, because there is no reason to assume the two
...
jack's ring responds across its full diameter. Both were once much smaller than
they appeared — an SVG shape only hit-tests where it is *painted*, so an unfilled
jack circle responded on about four live pixels. If you remember it feeling
fiddly, try again.
```
...
travel inside an exported patch file. If a hand layout has spread past anything the
frame can show, **snap** re-lays it from the signal chain instead of dutifully
pinning it somewhere you cannot see.

## Locks, and evolving from here

This is the feature that makes the rack more than an editor.
...
The guarantee is exact rather than best-effort. A proposal that would change,
delete **or create** any locked address is rejected — both directions, which is
not pedantry: checking only one lets a *birth* at a locked address through while
rejecting the death that would undo it, and a search that can enter a state it can
never leave will drift into locked structure and stay there. Excluding both makes
locking exactly conditional refinement.

One honest limit: a lock is a set of exact addresses, so a structural move that
grows a *brand-new* address inside a locked module — one that existed in neither
version, so it cannot be in the set — is not caught. That case is symmetric, so it
costs nothing in correctness; it just means "locked" is a promise about
...
chosen and lit, rather than reprinting the module list. One inventory, one place —
a palette and a right-click menu that describe different instruments is how an
editor starts lying to you.

Anything you bypass or delete goes to the **HELD** tray rather than disappearing,
