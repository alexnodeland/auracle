...
learned taste supplies direction, and $\beta$ is the single dial between
them.</p>
...
**$p_{\text{grammar}}(x)$ is the parsimony pressure.** It is the prior
...
*is* written down, and the parsimony term is a probability rather than a
hyperparameter.
...
**The standardizer must be the one the observations were made under.** $\theta$
is meaningless against any other scaling; see
...
**What ships is not a sample from $\pi_\beta$.** Refinement runs a short
adaptive single-site MH walk warm-started from each of the best pool members
