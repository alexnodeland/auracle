...
taste model was being asked to learn preferences over evidence that was not in
...
3. **C4+E4 dyad.** A second compiled voice, gate-synced with the main voice,
   reveals intermodulation (`chord_flatness_delta`). A dyad rather than a triad
   because render cost is per voice-second and pairwise intermodulation is the
   first-order phenomenon.
4. **C3 with a 1.1 s release window, kept last.** Bass register, and its
   position matters: the tail measurement is the final 300 ms, so putting this
...
So tagging the name with the stimulus generation means votes recorded under the
v1 phrase:
