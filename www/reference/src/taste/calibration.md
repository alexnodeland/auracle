...
A running count of $p > 0.5$ outcomes is **accuracy**, and accuracy is not a
proper scoring rule. Two failures, and the second is fatal here:
...
leave the diagonal. Then press <em>ask only near-ties</em>: the hit rate drops
to around 50% and stays there <em>however good the model is</em>, because that
is what asking hard questions does to a coin-flip tally. The skill score keeps
moving, which is why it is the one on display.</figcaption>
</figure>
...
Brier is proper and bounded, and it **moves as sharpness improves** rather than
only as accuracy does, which is the property accuracy lacked.
...
acquisition rules: an information-seeking rule serves duels near $p = 0.5$,
which carry the highest log-loss by construction. Comparing two rules on their
own self-chosen question sets would score *the willingness to ask hard
questions* as a failure.
...
uniform pairing was chosen. See [Acquisition](../search/acquisition.md).
```
...
Committing to a forecast before each answer and then reporting the error
against a proper scoring rule means the model **can publicly fail**, and early
on it does.
