...
//! `ricercar_taste::standardize`'s runaway threshold.
//!
//! ```bash
//! cargo run -p ricercar-features --example winsor_ratio --release
//! cargo run -p ricercar-features --example winsor_ratio --release -- 150  # pools
//! ```
//!
...
use ricercar_features::{featurize, Features, PhraseSpec};
use ricercar_grammar::PatchGrammarPrior;

/// The reference population `Engine::refit_standardizer` fits on.
const POOL: usize = 48;
/// Mirrors `ricercar_taste::standardize`'s tail fraction.
const TAIL: f64 = 0.02;
