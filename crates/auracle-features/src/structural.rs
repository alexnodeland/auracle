...
//! draws caught (`cargo run -p auracle-features --example pipeline_stats
...
//! recovered from the source coefficients in `auracle_session`'s
...
//! `auracle-session`'s `search_health` module doc, next to the harness that
...
use auracle_grammar::term::{AudioNode, ModNode, ModOp, PairOp, PatchTree};
