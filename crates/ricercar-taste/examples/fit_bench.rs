...
//!
//! ```text
//! cargo run --release -p ricercar-taste --example fit_bench            # 20k/6k, the wasm budget
//! cargo run --release -p ricercar-taste --example fit_bench -- 5000 1500
//! ```
//!
...
use ricercar_taste::{Feedback, FitSet, Observation, ObservationLog, TasteConfig, TasteModel};
...
fn checksum(p: &ricercar_taste::TastePosterior) -> u64 {
...
fn sweep(samples: usize, warmup: usize, k: usize, n_obs: usize, seeds: u64) {
    ...
    for seed in 0..seeds {
        ...
        let user = ricercar_taste::SyntheticUser {
            theta,
            tau: 0.0,
        ...
        cos_sum += (0..posterior.k_styles())
            .map(|kk| ricercar_taste::synthetic::cosine(&posterior.theta_mean(kk), &user.theta))
